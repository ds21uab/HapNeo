...
##     - source_data/clinical_holdout.csv
##       Columns: patient_id, OS_month, OS_status, age, sex, batch, PC1-PC5
##     - source_data/geno_holdout.csv
##       Columns: CHROM, POS, REF, ALT, var_id, <patient_id columns>
##
##   POPLAR (controlled access — available from EGA: EGAS00001002460):
##     - source_data/clinical_poplar.csv
##       Columns: patient_id, OS_months, OS_status, age, sex, PC1-PC5
##     - source_data/geno_poplar.csv
##       Columns: CHROM, POS, REF, ALT, var_id, <patient_id columns>
##
...
lead_snps     <- fread("results/gwas_discovery_results.csv")
sig_threshold <- 5e-4
...
validate_cohort <- function(geno_file, clinical_file, selected_snps,
  ...
  if ("OS_months" %in% colnames(clinical) & !"OS_month" %in% colnames(clinical)) {
    setnames(clinical, "OS_months", "OS_month")
  }
...
## clinical_poplar.csv and geno_poplar.csv in source_data/.
...
snps_holdout <- val_holdout[validated == TRUE]$SNP
snps_poplar  <- val_poplar[validated == TRUE]$SNP

# Variants validated in each cohort
cat("\n========================================\n")
